...
> **完整功能/接口/实现清单（新 GUI 参考）请优先阅读：** [`GUI_REDESIGN_REFERENCE.md`](GUI_REDESIGN_REFERENCE.md)。本文仅保留主窗口控件级笔记，部分 v1.2.2+ 入口可能未覆盖。
...
- 组头右键菜单：重命名文件夹（磁盘 + 内存原位重映射）、批量重命名图片、对该文件夹跑 ONNX / LLM 打标、替换透明背景（当前文件夹）、替换透明背景（全部图片）。
...
- Options：设置、语言切换。
- Tools：替换透明背景（选中图片）、视频格式转换、视频抽帧、ONNX 推标、背景移除、LLM 打标。
