...
namespace BooruDatasetTagManager
{
    ...
    public class MultiSelectDataTable : DataTable
    {
        ...
        private async Task TranslatePendingAsync(Func<string, Task<string>> translateAsync)
        {
            ...
            while (true)
            {
                var pending = Rows.Cast<MultiSelectDataRow>()
                    .Where(row => row.RowState != DataRowState.Deleted
                        && row.RowState != DataRowState.Detached
                        && !string.IsNullOrWhiteSpace((string)row["Tag"])
                        && string.IsNullOrWhiteSpace((string)row["Translation"]))
                    .Select(row => (Row: row, Tag: row.GetTagText()))
