...
namespace BooruDatasetTagManager
{
    public class AutoCompleteTextBox : TextBox
    {
        ...
        private void UpdateListBox()
        {
            ...
            bool isChineseInput = ContainsChineseCharacters(word);

            if (_mode == AutocompleteMode.Disable)
            {
                ResetListBox();
                return;
            }

            if (_values != null && word.Length > 0 && (isChineseInput || word.Length >= Program.Settings.ShowAutocompleteAfterCharCount))
            {
                TagsDB.TagItem[] matches = isChineseInput
                    ? Program.ChineseTagLookup.SearchAutocompleteValues(word, _values, Program.Settings.Language)
                    : null;
