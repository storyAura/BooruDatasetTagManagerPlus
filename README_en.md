...
| Module | Description |
| --- | --- |
| **LLM Settings** | OpenAI-compatible endpoint; separate text, auto-tag vision, and audit vision models; LLM concurrency and the fixed natural-language prompt |
...
| **Background removal** | Built-in RMBG-1.4 ONNX, runs locally in the client — **no external service**; one-click model download on first use (~176 MB, or ~44 MB quantized); transparent or solid-color background (white by default), overwrite the original or save a `_nobg.png` copy |
...
- **Unified LLM tagging window** — Tags / Tags→Natural-language (formerly TAG2NL) modes; output format (Tags+NL / NL only), `_captioned` copy or in-place write, ONNX-first on untagged images; prompt template and tagging settings folded into the window; "AI vision tagging" renamed to "LLM tagging" with a global LLM concurrency setting
...
- Read-only source data; atomic writes, cancel support, per-file error isolation, transactional rollback
...
4. Run **Tools → LLM tagging...** (Tags / Tags→Natural-language modes) or **Test → Open character tag audit...**
...
Built-in Danbooru Tag, Natural language, Hybrid mode, Natural language 2. Custom templates export as JSON without credentials.

## LLM tagging

**Tools → LLM tagging...**, right-click a dataset image → **LLM tagging**, or the tag-toolbar "Auto generate tags" button.
...
- **Tags mode** — image → tags, written back to the dataset per the write mode (replace / append / skip existing), with sort, prefix/suffix, and underscore post-processing.
- **Tags → Natural language mode (formerly TAG2NL)** — tags + image → a natural-language paragraph.
  - **Output format** — **Tags + natural language** (default, the original TAG2NL format) or **natural language only**.
  - **Caption output** — **Save a copy** (default) to `dataset_captioned/` (source `.txt` read-only, existing skippable) or **write in place** into the image's own `.txt` (through the dataset manager, so memory and disk stay consistent).
...
- Built-in RMBG-1.4 ONNX runs locally in the client — **no external service**; one click on "Download and load model" fetches it on first use (~176 MB, or ~44 MB quantized; official / mirror source), and a cached model auto-loads when the dialog opens
...
- **Green border = has the tag, red = missing**; click Y/N on a thumbnail to toggle
...
Running locally creates **Models/** (downloaded ONNX weights), **Cache/** (e.g. video thumbnails), and **settings.json** (API keys and preferences) beside the executable. These are locally generated data and safe to delete — settings reset to defaults, and models can be re-downloaded from inside the app.
