...
4. 按需运行 **工具 → LLM 打标 / ONNX 推标 / 背景移除 / 视频工具 / 查找相似图片**,或 **测试 → 打开角色标签审查**
...
- **标签着色与类别排序**:两个标签面板按 18 类语义浅色着色(角色 / 作品 / 发型 / 眼睛 / 服装…);图片标签工具栏的「类别排序」是记忆开关:勾选后每张选中图自动按分类分组排序(遵守「不排序前 N 行」),状态持久保存;全部标签面板的类别排序默认关闭,可在其工具栏勾选
