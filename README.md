...
- **1.2.2**(当前)— 新增:相似图片筛选(重复图按组清理)、角色标签审查多角色模式(最多 4 人)、标签面板类别筛选、数据集平铺视图、错误标签修复、全套命令行 CLI、重新导入当前数据集(F5)。修复:「跳过已有标签列表」不再静默丢结果白扣费(P0)、Ctrl+Z 撤销连环崩溃、未加载数据集时的空引用崩溃、换文件夹残留旧标签等。[发布说明](docs/RELEASE_NOTES_v1.2.2.md)
