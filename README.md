...
- **1.2.4**(当前)— 修复:多人角色审查「应用并保存」后左上角角色下拉不跟进、WD14 ONNX 通道顺序导致错色标签、超长文件名保存失败(原子写临时名过长)、LLM 打标超过 200 张时二次确认防误点;错误标签修复的子级并入改为默认关闭(测试模块可勾选,预览可逐条勾选);ONNX 推标按置信度排序而不再按 CSV 字母序;文件名以 `_` 结尾时打标/写 caption 不再报错。新增:视频抽帧可按百分比随机抽取(分布 / 区域两种模式,默认 10%);数据集可按类型(jpg / png / 视频等)排序。[发布说明](docs/RELEASE_NOTES_v1.2.4.md)
- **1.2.3** — 新增:扫描坏图(工具菜单,审查墙绿留红删)、替换透明背景支持按当前文件夹 / 全部图片批量(文件夹右键菜单)。修复:替换透明背景对 WebP 报内存溢出(改走 ImageSharp 解码 + 按扩展名编码)、标签筛选「点 NOT 得 OR」(下拉明选 AND/OR/NOT/XOR)、换标签立刻重筛、再点当前模式取消筛选。安全/I/O 加固:DPAPI 加密失败不再明文落盘密钥、LLM 图像 GDI 泄漏、文件夹重命名路径穿越、模型下载竞态、更新包文件名消毒、审查删除门控改本地分类、Caption/ffmpeg/主题原子写等。[发布说明](docs/RELEASE_NOTES_v1.2.3.md)
...
| 模块 | 说明 |
| --- | --- |
| **数据集浏览** | 文件夹分组浏览器(搜索、折叠、重命名 / 批量重命名、右键快捷推标);平铺视图;按类型/文件名/日期排序;内嵌预览(多选并排);行内显示格式·像素·大小 |
...
| **错误标签修复** | 人数标签冲突 / 多人 solo / 角色父子重复一键清理;子级并入父级默认关闭(测试模块可勾选);预览可逐条勾选、可撤销 |
| **视频处理** | 格式转换;全部帧 / 按 FPS / 指定帧 / 随机百分比抽帧(分布或区域);内置 FFmpeg |
