...
When this skill is used by the character tag auditor, it only ORDERS the prompt; every keep/delete/replace decision comes from the character-tag-auditor skill's procedure (Steps 1–8), and this skill must not contradict it. Emit only the locked character's core prompt: trigger first, then subject count, eye and hair colors, hair structure, head accessories, clothing from torso downward, worn accessories, footwear. Respect the auditor's `include_in_prompt` and `prompt_order` fields; protected scene, action, pose, expression, composition, quality, ordinary-object, and other-character traits stay out of the prompt even though they remain in dataset files.

The line must contain one canonical tag per feature slot, in the auditor's collapsed form: the most specific visually confirmed tag (`low twintails`, not `twintails` + `twin braids`; one colored hair ribbon or colored hairband, one colored jacket), color-prefixed garment, footwear, and accessory tags whenever the reference confirmed the color — even when the colored form never appeared in the original tag list (`black jacket`, `black boots`) — and a color-less tag only when its color was genuinely unverifiable. Composites may only be `color` + a real tag (`black hairband`); never invent `black elbow gloves` or `frilled black dress`. A precise colored tag already in the inventory is never replaced. One accessory, one tag. Container words are families, not features: `jewelry` never stands beside `earrings`, `bow`/`ribbon` never beside the `hair ribbon` they describe, `hair ornament` never beside a specific hair accessory, decoration words attach to their garment (`frilled dress`) or disappear. Bare `jewelry` is allowed only when no specific jewelry tag exists. Never create a color, garment, body trait, or accessory that the input and reference image do not support.
